import { binarySearch, binarySearchFirstEntryInsertionPosition, binarySearchLastEntry } from "../../src/search/binary"
...
describe('Binary', () => {
    ...
    describe('Binary First Entry Insertion Position', () => {
        test('Contains value', () => {
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 0)).toBe(0);
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 1)).toBe(1);
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 2)).toBe(2);
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 3)).toBe(3);
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 4)).toBe(4);
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 5)).toBe(5);
        });

        test('Returns first from the same values', () => {
            expect(binarySearchFirstEntryInsertionPosition([0,1,1,1,1,2], 1)).toBe(1);
            expect(binarySearchFirstEntryInsertionPosition([0,0,0,0,0,0], 0)).toBe(0);
        });

        test('Does Not Contain value returns insert position', () => {
            expect(binarySearchFirstEntryInsertionPosition([0,1,2,3,4,5], 8)).toBe(6);
        });
